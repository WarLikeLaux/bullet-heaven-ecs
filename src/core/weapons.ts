...
type Targetable = { position: Vector3; hp?: number; pendingDamage?: number };

function effectiveHp(e: Targetable): number {
  return (e.hp ?? 0) - (e.pendingDamage ?? 0);
}

export function findNearestEnemy(
  playerPos: Vector3,
  enemies: Iterable<Targetable>
): Targetable | null {
...
export function runAutoFireSystem(
  ...
  for (const player of players) {
    ...
    target.pendingDamage =
      (target.pendingDamage ?? 0) + (projectile.damage ?? 0);
